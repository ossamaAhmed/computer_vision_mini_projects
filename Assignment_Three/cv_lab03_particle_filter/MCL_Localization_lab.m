...
%
    ...
    % We will need to update the robot position here taking into account the
    % noise in acceleration
    %adapted from the simulate robot function.
    predicted_particles = [];
    curr_speed = uk;
    for i=1:size(priorbelief_particles, 2)
        wk = randn(1)*simpar.odometry_stdev;
        predicted_particles(i) = priorbelief_particles(i) + simpar.T*curr_speed + (simpar.T^2/2)* wk;
        if simpar.circularHallway
            % the hallway is assumed to be circular
            predicted_particles(i)=mod(predicted_particles(i),simpar.domain);
        else
            % the hallway is assumed to be linear
            if predicted_particles(i)>simpar.domain
                predicted_particles(i) = simpar.domain-mod(predicted_particles(i),simpar.domain);
                curr_speed = -curr_speed; % change direction of motion
            end
            if predicted_particles(k)<0
                predicted_particles(k) = -predicted_particles(k);
                curr_speed = -curr_speed; % change direction of motion
            end
        end
    end
    predict_particles = predicted_particles;
end
...
%
    ...
    % normalization
    particle_weights_sum = sum(particle_weights);
    particle_weights = particle_weights ./ particle_weights_sum;
end
...
%
% COMPLETE THIS FUNCTION
%Adapted from Udacity class

    % change this for the correct method
    % Important note!! It is not allowed to use randsample()
    result = []
    %get a random number
    index = randi(size(predict_particles, 2));
    beta = 0;
    max_weight = max(particle_weights);
    for i=1:size(predict_particles, 2)
        beta = beta + rand(1)*2.0*max_weight;
        while beta > particle_weights(1, index)
            beta = beta - particle_weights(1, index);
            index = mod(index+1, size(predict_particles, 2));
            if index == 0
                index=1;
            end
        end
        result = [result predict_particles(1, index)]; %add it to the result
    end
    updated_belief_particles = result;
end
